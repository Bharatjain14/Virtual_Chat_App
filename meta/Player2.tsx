...
function getCoordinates(){
    const dbref = ref(db);
    get(child(dbref,"coordinates/3001")).then((snapshot)=>{
        if(snapshot.exists()){
            x=snapshot.val().x;
            z=snapshot.val().y;
            //console.log(x);
...
const Player = ({avatar}) =>{
  ...
  const {
    ...
    //console.log("http://localhost:3000/"+avatar+".glb");
    
     
    
    //const data =  contract.getAvatar(account);
    //console.log(data);
    ...
    {
      l=message.length;
      let ad = message[l-1].substring(0,42);
      //alert(currentUserAddress.toLowerCase()+" "+ad);
      
      //console.log("in");
      //console.log(message[l-1].substring(0,42));
      //console.log(avatar);
      if(message[l-1].substring(0,42)===currentUserAddress.toLowerCase())
      {
        //alert("got player2");
